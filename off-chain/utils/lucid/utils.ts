import { NETWORK, CREDENTIALS_PATH } from "../../common/constants.ts";
...
export async function generateNewWallet(walletName: string, walletPath?: string) {
...
export function parseValidatorAndApplyParameters(validators: any, params: [any], title: string): AppliedValidator {
    const validator = parseValidator(validators, title);
    return applyValidatorParameters(validator, params, title);
}

export function applyValidatorParameters(rawValidator: Script, params: [any], title: string): AppliedValidator {
